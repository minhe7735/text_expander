...
struct expansion_pair {
    char short_code[MAX_SHORT_LEN];
    char expanded_text[MAX_EXPANDED_LEN];
    bool active;
};

struct text_expander_data {
    struct expansion_pair expansions[MAX_EXPANSIONS];
...
int zmk_text_expander_add_expansion(const char *short_code, const char *expanded_text) {
    ...
    for (int i = 0; i < MAX_EXPANSIONS; i++) {
        if (expander_data.expansions[i].active && 
            strcmp(expander_data.expansions[i].short_code, short_code) == 0) {
            // Update existing expansion
            strncpy(expander_data.expansions[i].expanded_text, expanded_text, MAX_EXPANDED_LEN - 1);
            expander_data.expansions[i].expanded_text[MAX_EXPANDED_LEN - 1] = '\0';
            k_mutex_unlock(&expander_data.mutex);
            LOG_INF("Updated expansion: %s -> %s", short_code, expanded_text);
            return 0;
        }
    }
    ...
    for (int i = 0; i < MAX_EXPANSIONS; i++) {
        if (!expander_data.expansions[i].active) {
            strncpy(expander_data.expansions[i].short_code, short_code, MAX_SHORT_LEN - 1);
            expander_data.expansions[i].short_code[MAX_SHORT_LEN - 1] = '\0';
            strncpy(expander_data.expansions[i].expanded_text, expanded_text, MAX_EXPANDED_LEN - 1);
            expander_data.expansions[i].expanded_text[MAX_EXPANDED_LEN - 1] = '\0';
            expander_data.expansions[i].active = true;
            expander_data.expansion_count++;
            k_mutex_unlock(&expander_data.mutex);
            LOG_INF("Added expansion: %s -> %s", short_code, expanded_text);
            return 0;
        }
    }
    
    k_mutex_unlock(&expander_data.mutex);
    LOG_WRN("Maximum expansions reached (%d)", MAX_EXPANSIONS);
    return -ENOMEM;
}
...
int zmk_text_expander_remove_expansion(const char *short_code) {
    ...
    for (int i = 0; i < MAX_EXPANSIONS; i++) {
        if (expander_data.expansions[i].active && 
            strcmp(expander_data.expansions[i].short_code, short_code) == 0) {
            memset(&expander_data.expansions[i], 0, sizeof(struct expansion_pair));
            expander_data.expansion_count--;
            k_mutex_unlock(&expander_data.mutex);
...
void zmk_text_expander_clear_all(void) {
    ...
    memset(expander_data.expansions, 0, sizeof(expander_data.expansions));
    expander_data.expansion_count = 0;
    
    k_mutex_unlock(&expander_data.mutex);
    LOG_INF("Cleared all expansions");
}
...
static const char* find_expansion(const char *short_code) {
    for (int i = 0; i < MAX_EXPANSIONS; i++) {
        if (expander_data.expansions[i].active && 
            strcmp(expander_data.expansions[i].short_code, short_code) == 0) {
            return expander_data.expansions[i].expanded_text;
        }
    }
    return NULL;
}
